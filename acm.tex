...
\newcommand\downrepro{Downstream Reproduction}:
...
However, these approaches mostly frame the problem as an output prediction task, which (1) ignores how the output is obtained and (2) cannot distinguish between agent errors and artifact errors when results mismatch.
To address this, we present \approach, an automated artifact evaluation approach that frames the task as a code generation task.
...
If the results mismatch, it could be that the error is on the side of either the paper or the artifact, which we term \emph{\newbug}.
To evaluate \approach, we introduce \benchmark, a new benchmark evaluating LLM agents' capabilities to generate \goodscript.
...
Furthermore, applying \approach~to~\benchmark~uncovered~\fastpathsize~\fastpath{}~and \newbugsize~\newbug{}, demonstrating the strength of our approach.
\end{abstract}
\maketitle
