...
\approach~with gpt-5.1 gives the best result, successfully reproducing the expected table in~\goodscripttsize~out of the 60 tasks (20 full reproduction and 21 last-mile reproduction).
The second-best result is from~\approach~with gpt-5-mini with 26 successful reproductions out of 60 (12 full reproduction and 14 last-mile reproduction), even outperforming baselines with gpt-5.1.
\approach~with gpt-5-nano effectively decreases the rate of static error to 8 out of 60 (compared to 49 / 60 for SWE-agent and 60 / 60 for mini-swe-agent) but does not lead to substantial increase in successful reproduction (only 1 / 60 success).
...
For 7 / 16 tasks, Get tool fails to download the artifact due to network error for one case.
As we target the live artiafct repositories as our target, some of the instabilities are inavoidable.
During the development, we even encountered a Cloudflare outrage that prevented us from evaluating our approach \cite{cloudflare}.
Although disabled for fair evaluation, the caching mechanism of the Get tool (Section~\ref{s:get}) would help mitigate the network errors in the in-the-wild usage.
...
Comparing~\approach~with less powerful LLMs with baselines show an interesting trend.
\approach~with gpt-5-mini is more efficient in terms of cost, costing \$0.137 per instance, compared to mini-swe-agent with gpt-5.1 (\$0.285 per instance) and SWE-agent with gpt-5.1 (\$0.123 per instance), while being more effective.
...
\begin{table}[ht]
    ...
    \begin{minipage}{0.48\textwidth}
        \centering
        \caption{Fidelity of LLM-based Format Tool}
...
We study the fidelity of the Format tool.
\todo.

\subsubsection{RQ4-2: Accuracy of the Judge tool}
\label{s:eval_judge}

We study the accuracy of the LLM-based Judge tool.
\todo.
