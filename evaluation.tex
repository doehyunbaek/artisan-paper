...
\subsubsection{Agent Framework and User Prompt}
...
5. **Author the reproduction script** `repro_action_table_6.sh` using the template below.
  \end{minted}
  \caption{Example user prompt provided to the mini-swe-agent tasked with reproducing Table 6 of Bouzenia and Pradel~\cite{DBLP:conf/icse/BouzeniaP24}.}
  \Description{Example user prompt provided to the mini-swe-agent tasked with reproducing Table 6 of Bouzenia and Pradel~\cite{DBLP:conf/icse/BouzeniaP24}.}
  \label{f:user_prompt}
\end{figure}
...
\approach~can be understood as the baseline mini-swe-agent plus the three tools that we present in Section~\ref{s:approach}, which we provide with the added user prompts shown in Figure~\ref{s:line_diffs}.
We choose SWE-agent as an established state-of-the-art approach for general software engineering tasks.
To adopt SWE-agent to the task of~\benchmark, we give the same user prompt as in mini-swe-agent but adds one last instruction to the workflow to adopt SWE-agent's hard-coded APR task to our task.
\footnote{6. Copy the submission script: \texttt{cp repro\_\{\{ artifact\_name \}\}\_table\_\{\{ table\_index \}\}.sh /root/model.patch}}
For both baselines and~\approach, we experiment with gpt-5.1-2025-11-13, gpt-5-mini-2025-08-07, and gpt-5-nano-2025-08-07 as an LLM model, and use the step limit of 30 and cost limit of one dollar per task.
...
\todo{Should we also manually validate Mismatch Results in case they are correct? Due to format tool non-determinism.}
