...
(2) The badge is an one-time assessment: The badge signals that the artifact worked at the time of manual artifact evaluation, but does not imply it works now.
...
Motivated by this limitation, we pose the question: \emph{What if we task LLM agents with generating code that reproduces the results}?
...
\approach~differs from the previous work by using~\goodscript~as an output of agents.
This enables judging of the script, which can be utilized to discover both~\newbug~and~\fastpath.
