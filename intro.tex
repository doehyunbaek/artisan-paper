...
\toprule
 & \textbf{SUPER}~\cite{SUPER} & \textbf{CORE-Bench}~\cite{DBLP:journals/tmlr/SiegelKNSN24} & \textbf{Repro-Bench}~\cite{DBLP:conf/acl/HuZLWPK25} & \textbf{\benchmark~(Ours)} \\
\midrule
...
SUPER~\cite{SUPER} evaluates the ability of LLM agents to reproduce results from ML and NLP research repositories.
...
% One limitation we identify is that in the previous benchmark, artifact (research repositories according to the previous work) are treated in isolation, neglecting its relationship with the research paper.
% For example, SUPER~\cite{SUPER} start from the research repositories to construct the tasks and not consult the research paper.
% This measures consistency of the reproduction of the research repositories with some reference solution, but the reproducibility of the findings of the research paper itself is neglected.
...
Our evaluation shows that while most LLM agents struggle on \benchmark, Artisan is the most effective, producing \goodscripttsize{} reproduction scripts and outperforming the baseline by \goodscripttsizeoutperform{}.
During the construction of \benchmark, we also uncovered \newbugsize~inconsistencies between papers and artifacts and \fastpathsize~fast paths that artifacts provide, demonstrating the strength of our approach to benchmark construction.

In summary, this paper contributes the following:
\begin{itemize}
    \item Artisan, an LLM agent which automates artifact evaluation through code generation.
    ...
    \item Empirical evaluation which shows that Artisan generates \goodscripttsize{} new \goodscript{}.
