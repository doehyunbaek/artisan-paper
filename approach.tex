...
When we provide a table to the the LLM agent, we provide the table with digits obfuscated with question marks.
While developing our approach, we found that giving the table unobfuscated led to the undesirable behavior of directly copying the given results.
...
% The same obfuscated table is used for the provided paper as well.
...
\todo{Decide on tools to include in approach}

\subsubsection{Agents struggle with fetching research repositoreis}
\label{s:get_challenge}
...
Figure~\ref{f:overview}~gives overview of~\approach.
Per the problem definition (Section~\ref{s:problem_def}), a user of~\approach~provide paper, table, and artifact to~\approach, with the table obfuscated.
Then,~\approach, an LLM agent based approach tries to generate a reproduction script, which is a bash script that reproduces the table from scratch using the artifact.
An LLM agent is an LLM-based system that uses tools to autonomously interact with the environment to achieve a given goal~\cite{DBLP:conf/icse/BouzeniaDP25}.
As a user prompt, the LLM agent is provided with a series of steps that should be taken to reproduce a numerical result from the research paper.
...
The workflow provided to the agent is modeled after how researchers would tackle the problem of reproducing research artifacts themselves.

LLM agent is provided with three tools to achieve its goal.
First, LLM agent is provided with Bash tool~\cite{minisweagent}~as a Swiss Army knife: the agent performs all regular CLI use tasks like reading a file, writing a file, running commands, using the single Bash tool.
In addition,~\approach~provides two tools developed for the domain-specific challenges (Section~\ref{s:challenges}) of automated artifact evaluation.
First, to aid the agent in analyzing and formatting the execution logs into the expected format (Section~\ref{s:format_challenge}),~\approach~provides the Format tool (Section~\ref{s:format}), which is an LLM-based formatter that takes an expected results and execution log as input and outputs a formatted results.
Second, to aid the agent in choosing appropriate means of reproduction (Section~\ref{s:judge_challenge}),~\approach~provides the Judge tool (Section~\ref{s:judge}) which gives feedback on the \goodscript~submitted.
...
High-level algorithm is described in Algorithm~\ref{alg:artisan}, which will be explained in detail in the following sections.

\begin{algorithm}[t]
\caption{High-level algorithm of Artisan}
\label{alg:artisan}
\begin{algorithmic}[1]
\State \textbf{Input:} Paper $p$, Obfuscated table $t$, Artifact $a$
\State \textbf{Output:} Reproduction script $s$
\State $\text{step\_limit} \gets 30, \ H \gets []$
\While{$\text{step\_limit} > 0$}
    \State $\text{thought}, \text{cmd} \gets \text{LLM}(\text{SystemPrompt}, \text{UserPrompt}, H)$

    \If{$\text{cmd} \neq \text{"submits"}$}
        \State $\text{obs} \gets \text{execute}(\text{cmd})$
    \Else
        \State $s \gets \text{read\_script}()$
        \If{$J_{exec}(s) \neq \text{OK}$} \Comment{\textbf{Step 1: Execution-Based Output Check}}
            \State $\text{obs} \gets \text{Warning}(\text{"Syntax or Runtime Error"})$
        \Else
            \If{$J_{method}(s) == \text{CopyRepro}$} \Comment{\textbf{Step 2: LLM-Based Method Check}}
                \State $\text{obs} \gets \text{Warning}(\text{"Method rejected: No credit for CopyRepro."})$
            \Else
                \State \Return $s$ \Comment{Validated Success}
            \EndIf
        \EndIf
    \EndIf

    \State $H.\text{append}(\text{thought}, \text{cmd}, \text{obs})$
    \State $\text{step\_limit} \gets \text{step\_limit} - 1$
\EndWhile
\end{algorithmic}
\end{algorithm}
